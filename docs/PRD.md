...
公网接口基址为 `http://54.172.101.190/roto-kb/`（具备域名后升级 HTTPS）。查询接口使用 `Authorization: Bearer <ROTO_KB_READ_TOKEN>`；reload/release 等写接口使用权限独立的 `ROTO_KB_ADMIN_TOKEN`。
...
- 公网入口由 nginx 提供 `/roto-kb/` 路径；8710、6334 仅监听 loopback，不直接暴露公网。
...
| `ROTO_KB_PUBLIC_BASE_URL` | `http://localhost:8710` | `http://54.172.101.190/roto-kb` | 否 | 有域名后改 HTTPS |
