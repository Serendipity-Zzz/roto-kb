...
V1 只接收无需付费 API、无需运行时 API key、可离线解析、许可可确认、能够直接改善单位/来源/工业语义的内容。外部 RDF 的本体语义用于检索、映射和校验，不作为工程材料许用值或求解结论。
...
| IEEE 1872 CORA | link-and-map-only | 社区仓库无明确再分发许可证；不复制 OWL |
| MatOnto、PropNet | link-and-map-only / rejected | 许可证不清晰或 NOASSERTION |
...
2026-09-09 使用 `rdflib 7.1.4` 离线解析全部白名单文件成功，共 86,721 triples：W3C 三份 3,969、QUDT 四份 78,520、IOF Core 4,232。ROTO seed 共 62 条记录（41 nodes、18 relations、3 rules），无重复 node ID、无悬空关系、无未知 `source_refs`，且不含被排除的 API/许可未决来源引用。知识目录 563 个静态文件的 SHA-256 已重新生成并逐项校验。
